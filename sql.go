...
func main() {
	...
	for {
		// Read tokens from the XML decoder
		...
		case xml.StartElement:
			// Check if it's the "Statistik" element
			if se.Name.Local == "Statistik" {
				// Decode the XML element into a Statistik struct
				var statistik Statistik
				err := decoder.DecodeElement(&statistik, &se)
				if err != nil {
                ...
                statistik.KoeretoejIdent = trimAndSetEmptyToNull(statistik.KoeretoejIdent) //YC

				if (statistik.KoeretoejIdent != "NULL") {
					_, err = koeretoejStatement.Exec(
					...
					if (statistik.KoeretoejArtNavn != "NULL" ||
						...
						_, err = koeretoejArtStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.KoeretoejAnvendelseNavn != "NULL" ||
						...
						_, err = koeretoejAnvendelseStatement.Exec(
							statistik.KoeretoejIdent,
					...
					for i := range statistik.KoeretoejAnvendelseSamling.KoeretoejAnvendelse {
						...
						if (statistik.KoeretoejAnvendelseSamling.KoeretoejAnvendelse[i].KoeretoejAnvendelseNavn != "NULL" ||
							...
							_, err = koeretoejAnvendelseSupplerendeStatement.Exec(
								statistik.KoeretoejIdent,
					...
					if (statistik.LeasingMaaneder != "NULL" ||
						...
						_, err = koeretoejLeasingStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.RegistreringNummerIdent != "NULL" ||
						...
						_, err = koeretoejRegistreringNummerStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.RegistreringNummerRettighedGyldigFra != "NULL" ||
						...
						_, err = koeretoejRegistreringNummerRettighedStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.KoeretoejOplysningOprettetUdFra != "NULL" ||
						...
						_, err = koeretoejOplysningStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.KoeretoejMaerkeTypeNavn != "NULL" ||
						...
						_, err = koeretoejMaerkeTypeStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.KoeretoejModelTypeNavn != "NULL" ||
						...
						_, err = koeretoejModelTypeStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.KoeretoejTypeTypeNavn != "NULL" ||
						...
						_, err = koeretoejTypeTypeStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.KoeretoejVariantTypeNavn != "NULL" ||
						...
						_, err = koeretoejVariantTypeStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.FarveTypeNavn != "NULL" ||
						...
						_, err = koeretoejFarveTypeStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.KarrosseriTypeNavn != "NULL" ||
						...
						_, err = koeretoejKarrosseriTypeStatement.Exec(
							statistik.KoeretoejIdent,
					...
					for i := range statistik.KoeretoejSupplerendeKarrosseriSamling.KoeretoejSupplerendeKarrosseriTypeStruktur {
						...
						if (statistik.KoeretoejSupplerendeKarrosseriSamling.KoeretoejSupplerendeKarrosseriTypeStruktur[i].SupplerendeKarrosseriTypeNavn != "NULL" ||
							...
							_, err = koeretoejSupplerendeKarrosseriTypeStatement.Exec(
								statistik.KoeretoejIdent,
					...
					if (statistik.NormTypeNavn != "NULL" ||
						...
						_, err = koeretoejNormTypeStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.KoeretoejMiljoeOplysningCO2Udslip != "NULL" ||
						...
						_, err = koeretoejMiljoeOplysningStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.CO2EmissionKlasseNavn != "NULL" ||
						...
						_, err = koeretoejCO2EmissionKlasseStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.KoeretoejMotorCylinderAntal != "NULL" ||
						...
						_, err = koeretoejMotorStatement.Exec(
							statistik.KoeretoejIdent,
					...
					for i := range statistik.KoeretoejDrivmiddelSamling.DrivmiddelStruktur {
						...
						if (statistik.KoeretoejDrivmiddelSamling.DrivmiddelStruktur[i].KoeretoejMotorBraendselscelle != "NULL" ||
							...
							_, err = koeretoejDrivmiddelStatement.Exec(
								statistik.KoeretoejIdent,
						...
						if (statistik.KoeretoejDrivmiddelSamling.DrivmiddelStruktur[i].KoeretoejMotorMaaleNormTypeNavn != "NULL" ||
							...
							_, err = koeretoejMaaleNormStatement.Exec(
								statistik.KoeretoejIdent,
						...
						if (statistik.KoeretoejDrivmiddelSamling.DrivmiddelStruktur[i].DrivkraftTypeNavn != "NULL" ||
							...
							_, err = koeretoejDrivkraftTypeStatement.Exec(
								statistik.KoeretoejIdent,
						...
						if (statistik.KoeretoejDrivmiddelSamling.DrivmiddelStruktur[i].KoeretoejMotorKmPerLiter != "NULL" ||
							...
							_, err = koeretoejBraendstofStatement.Exec(
								statistik.KoeretoejIdent,
						...
						if (statistik.KoeretoejDrivmiddelSamling.DrivmiddelStruktur[i].KoeretoejMotorElektriskForbrug != "NULL" ||
							...
							_, err = koeretoejElforbrugStatement.Exec(
								statistik.KoeretoejIdent,
					...
					for i := range statistik.DispensationTypeSamling.DispensationTypeStruktur {
						...
						if (statistik.DispensationTypeSamling.DispensationTypeStruktur[i].DispensationTypeNummer != "NULL" ||
							...
							_, err = koeretoejDispensationTypeStatement.Exec(
								statistik.KoeretoejIdent,
					...
					for i := range statistik.KoeretoejUdstyrSamling.KoeretoejUdstyrStruktur {
						...
						if (statistik.KoeretoejUdstyrSamling.KoeretoejUdstyrStruktur[i].KoeretoejUdstyrAntal != "NULL" ||
							...
							_, err = koeretoejUdstyrStatement.Exec(
								statistik.KoeretoejIdent,
					...
					for i := range statistik.KoeretoejBlokeringAarsagListe.KoeretoejBlokeringAarsag {
						...
						if (statistik.KoeretoejBlokeringAarsagListe.KoeretoejBlokeringAarsag[i].KoeretoejBlokeringAarsagTypeNavn != "NULL" ||
							...
							_, err = koeretoejBlokeringAarsagStatement.Exec(
								statistik.KoeretoejIdent,
					...
					if (statistik.PrisOplysningerMindsteBeskatningspris != "NULL" ||
						...
						_, err = koeretoejPrisOplysningerStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.KoeretoejGruppeNavn != "NULL" ||
						...
						_, err = koeretoejGruppeStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.KoeretoejUndergruppeNavn != "NULL" ||
						...
						_, err = koeretoejUndergruppeStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.AdresseFortloebendeNummer != "NULL" ||
						...
						_, err = koeretoejAdresseStatement.Exec(
							statistik.KoeretoejIdent,
						...
						if err != nil {
							...
							content = fmt.Sprintf("Error executing SQL koeretoejAdresseStatement: %s", err)

							discordwebhook.SendMessage(discordWebhook, discordwebhook.Message{
								Username: &username,
								Content:  &content,
							})
					...
					*/
					...
					if (statistik.SynResultatNummer != "NULL" ||
						...
						_, err = koeretoejSynResultatStatement.Exec(
							statistik.KoeretoejIdent,
					...
					if (statistik.KoeretoejRegistreringGyldigFra != "NULL" ||
						...
						_, err = koeretoejRegistreringStatement.Exec(
							statistik.KoeretoejIdent,
					...
					for i := range statistik.Tilladelse.TilladelseStruktur {
						...
						if (statistik.Tilladelse.TilladelseStruktur[i].TilladelseGyldigFra != "NULL" ||
							...
							_, err = koeretoejTilladelseStatement.Exec(
								statistik.KoeretoejIdent,
						...
						if (statistik.Tilladelse.TilladelseStruktur[i].TilladelseTypeNavn != "NULL" ||
							...
							_, err = koeretoejTilladelseTypeStatement.Exec(
								statistik.KoeretoejIdent,
				...
				/*
				// Concatenate KoeretoejUdstyrTypeNavn values
				var udstyrNavnConcat string
				for _, udstyr := range statistik.KoeretoejUdstyrSamling.KoeretoejUdstyr {
					if udstyr.UdstyrTypeNavn != "" {
						udstyrNavnConcat += udstyr.UdstyrTypeNavn + ", "
					}
				}
				udstyrNavnConcat = strings.TrimSuffix(udstyrNavnConcat, ", ")

				// Concatenate KoeretoejOplysningOevrigtUdstyr and KoeretoejUdstyrTypeNavn
				combinedUdstyr := udstyrNavnConcat + ", " + statistik.KoeretoejOplysningOevrigtUdstyr 

				// Remove leading and trailing comma if present
				combinedUdstyr = strings.Trim(strings.TrimSpace(combinedUdstyr), ",")
				...
				*/
				...
				remainingRows := totalStatistikCount - statistikCount
				averageTimePerRow := elapsedTime / time.Duration(statistikCount)
				estimatedRemainingTime := averageTimePerRow * time.Duration(remainingRows)

				fmt.Printf("\rRow inserted (%d/%d) | Estimated remaining time: %s", statistikCount, totalStatistikCount, estimatedRemainingTime)
			}
		}
	}
	...
	fmt.Printf("\nData inserted into MySQL | Elapsed time: %s", elapsedTime)
}
